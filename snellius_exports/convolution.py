...
class CliffordSteerableConv(nn.Module):
    ...
    def __call__(self, x):
        ...
        """
        # Initializing kernel
        defkern_start = time.perf_counter()
        ...
        defken_stop = time.perf_counter()
        print("Default kernel init time", (defken_stop - defkern_start))
        ...
        if self.bias:
            output = output + bias
        defconv_end = time.perf_counter()
        print("Default Convolution time", (defconv_end - defkern_start))
...
class BatchlessConditionedCliffordSteerableConv(nn.Module):
    ...
    def __call__(self, x, condition):
        ...
        print("conditioned kernel plus condition", (time.perf_counter() - condition_start))
        ...
        if self.bias:
            output = output + bias
        condconv_end = time.perf_counter()
        print("Batchless conv time:", (condconv_end - condition_start))
        return output
